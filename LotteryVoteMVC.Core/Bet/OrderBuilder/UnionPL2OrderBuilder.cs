...
{
    internal class UnionPL2OrderBuilder : OrderBuilderBase<BetItem>
    {
        protected string Ext1 { get; set; }

        private List<CompanyType> _companyTypes = new List<CompanyType> { CompanyType.EighteenA, CompanyType.EighteenB, CompanyType.EighteenC };
        internal CompanyType CompanyA { get; set; }
        internal CompanyType CompanyB { get; set; }
            ...
            try
            {
                ...
                foreach (var betItem in betList)
                {
                    ...
                    for (int i = 0; i < betItem.CompanyList.Length; i += 2)
                    {
                        ...
                        if (!_companyTypes.Contains(company.CompanyType)) continue; //A&B PL2的第一个公司必须是18A类型的公司

                        var companyB = todayLotteryCompany.Find(it => it.CompanyId == betItem.CompanyList[i + 1]);
                        if (companyB == null || !_companyTypes.Contains(companyB.CompanyType) || company.CompanyType == companyB.CompanyType) continue;            //A&B PL2的第二个公司必须是18B
