$(document).ready(function () {
    ...
    $(".allChecked").bind("change", function () {
        var chk = $(this);
        var companys = chk.attr("id").split("_");
        var companyA = companys[1];
        var companyB = companys[2];
        var companyArr = new Array();
        for (var i = 1; i <= 10; i++) {
            $(".company" + i).each(function (i, obj) {
                obj = $(obj);
                if (obj.attr("company") == companyA && obj.attr("companyb") == companyB) {
                    companyArr.push(obj.parent("span"));
                }
            });
        }
...
var parseToRollParlay = function (notNullAmount) {
    ...
    notNullAmount.each(function (i, obj) {
        ...
        var rowId = obj.attr("rowid");
        var nums = $(".num" + rowId);
        if (nums.length != 2) return true;     //号码个数必须是2个，否则跳过
        ...
        company.each(function (x, chk) {
            ...
            checkCompany.push(chk.attr("companyb"));
            var numStr = notNullNumArr.join("#");
            var companyStr = "[" + checkCompany.join() + "]";
            var wargerStr = "[" + buildWarger(chk.attr("game"), rowId) + "]";
...
var buildWarger = function (gamePlayType, rowId) {
    //    var gamePlayType = $("#PL2").val();
    var amount;
...
var CalcBetAmount = function () {
    ...
    $(notNullAmountArr).each(function (i, obj) {
        ...
        var numCount = calcBetNumCount(betCompanys) * 2;    //因为是AB所以要乘以2
