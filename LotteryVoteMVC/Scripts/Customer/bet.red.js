$(function () {
    ...
    var numCheck = function ($this, len) {
        if ($this.val().length == len)
            $this.closest("tr").find("input").removeAttr("disabled");
        else
            $this.closest("tr").find("input").not(".num").attr("disabled", "disabled");

    };
    var check2d = function () {
        numCheck($(this), 2);
    }
    $(".2d-num").keyup(check2d).blur(check2d);
    ...
    $(companys).each(function (t, x) {
        ...
        $(wagerArr).each(function (i, obj) {
            ...
            if (!window.Commission || !window.Commission[commKey]) {
                getCommission(comNumLen.CompanyId, gameplaywayInfo[0]);
                return false; //终止循环
            }
